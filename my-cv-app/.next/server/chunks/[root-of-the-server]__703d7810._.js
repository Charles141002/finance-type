...
"[project]/Desktop/finance-type/my-cv-app/pages/api/cv/detail.ts [api] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
const handler = async (req, res)=>{
    ...
    if (!id) {
        return res.status(400).json({
            error: 'Identifiant du CV manquant.'
        });
    }
    const { data, error } = await supabase.from('user_cvs').select('id, title, blocks, font_scale, created_at, updated_at').eq('user_id', session.user.id).eq('id', id).maybeSingle();
    if (error) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__703d7810._.js.map